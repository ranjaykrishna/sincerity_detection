...
l <- fit$loadings
write.table(l, 'fact_anal/maleLoadingsMatrix.csv', sep = ';', col.names = TRUE, row.names = TRUE)
...
fit <- factanal(fa_data, 5, rotation = 'varimax')
l <- fit$loadings
write.table(l, 'fact_anal/femLoadingsMatrix.csv', sep = ';', col.names = TRUE, row.names = TRUE)
...
for(col in 15:15){ #Iterate through output columns
...
write.table(final_scores, 'female_all_new_results.csv', sep = ';')
